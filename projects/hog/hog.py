...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    have_one_number = False
    score = 0
    while num_rolls > 0:
        curr = dice()
        if curr == 1:
            have_one_number = True
        else:
            score = score + curr
        num_rolls -= 1
    if have_one_number:
...
def is_always_roll(strategy, goal=GOAL):
    ...
    """
    # BEGIN PROBLEM 7
    init_strategy = strategy(0, 0)
    score0 = 0
    while score0 <= goal:
        score1 = 0
        while score1 <= goal:
            if strategy(score0, score1) != init_strategy:
                return False
            score1 += 1
        score0 += 1
    return True
...
def make_averaged(original_function, times_called=1000):
    ...
    # BEGIN PROBLEM 8
    def make_averaged_original(*args):
        i = 1
        total = 0
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    max_average_score = 0
    while now_rolls <= 10:
        now_average_score = make_averaged(roll_dice, times_called)(now_rolls, dice)
        if now_average_score > max_average_score:
            max_average_score = now_average_score
            best_rolls = now_rolls
        now_rolls += 1
    return best_rolls
    # END PROBLEM 9
